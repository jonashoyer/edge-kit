...
async function addVideoToLibrary(videoId: string, playlistId?: string) {
  ...
  await storage.upload(`videos/${videoId}.json`, Buffer.from(JSON.stringify(videoData, null, 2)));
...
async function searchVideoSegments(query: string, limit: number = 5) {
  ...
  const results = await vectorDb.query('video-segments', queryEmbedding, limit, { includeMetadata: true });
  ...
  return results.map((result) => ({
    ...
    videoUrl: youtubeService.getVideoUrlWithTimestamp(result.metadata!.videoId, result.metadata!.start),
  }));
}
```
...
```typescript
import { TranscriptSegment, YouTubeService } from '../services/youtube/youtube-service';
