 ...
 */
export class PosthogAnalytics<T extends Record<string, Record<string, unknown>>>
  implements AbstractAnalytics<T>
{
  constructor(token: string, config?: Partial<PostHogConfig>, name?: string) {
    posthog.init(
      token,
      { api_host: config?.api_host ?? 'https://eu.i.posthog.com', ...config },
      name
    );
  }
