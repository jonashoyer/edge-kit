...
export class YoutubeTranscriptHandler {
  ...
  async fetchRecord(url: string) {
    ...
    return {
      src: url,
      transcript,
      format: 'transcript/plain' as const,
      metadata: {
  ...
  async transcriptYoutubeVideo(videoId: string) {
    ...
    return {
      ...
      autoGenerated: transcriptData.transcript.content.footer?.language_menu?.sub_menu_items?.find(e => e.selected == true)?.title.includes('auto-generated'),
    }
  }
