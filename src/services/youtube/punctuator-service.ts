...
export class PunctuatorService {
  ...
  async punctuate(text: string) {
    ...
    const predictions = (await this.pipeline(segments.map(e => e.text.join(' ')))) as Prediction[][];

    return {
      text: this.punctuateSegment(predictions.flat()).replace(/\s+/g, ' ').replace(/\s'\s/g, '\'').trim(),
      predictions
    }
  };
  ...
  punctuateSegments(texts: string[]) {
    return texts.reduce(async (pAcc, text) => {
      ...
      return [...acc, this.punctuateSegment(predictions.flat()).replace(/\s+/g, ' ').replace(/\s'\s/g, '\'').trim()];
    }, Promise.resolve<string[]>([]));
  }

  private splitToSegments(words: string[], length: number, overlap: number): Segment[] {
    return Array(Math.ceil(words.length / length)).fill(0).map((_, i) => ({
